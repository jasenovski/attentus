...
def main():
    ...
    qtd_horas_total = st.sidebar.number_input(
        "Qtd. de horas totais do dia para trabalhar",
        min_value=5, max_value=48, value=24, step=1
    )
    st.sidebar.divider()
    ...
    turnos_possivies = lista_turnos_possiveis(1, qtd_horas_total, 
                                              minutos_dividir)
    
    dict_turnos_possiveis = {}
    for turno in turnos_possivies:
        horas = int(turno)
    ...
    for i in range(qtd_turnos):
        ...
        encargo = col2.number_input(
            f"Encargo do tunro {i + 1}",
            min_value=100, max_value=5000, value=540
        )
    ...
    df_nad, df_tabela_ligantes = gera_tabelas(
        qtd_horas_total, minutos_dividir, hora_inicial, minuto_inicial,
        turnos, encargos
    )
    ...
    arquivo_ligantes = st.sidebar.file_uploader(
        "Carregar arquivo de ligantes por periodo preenchidos",
        type=["xlsx"]
    )
    ...
    opcao = st.sidebar.segmented_control(
        options=["Ajustar Polinomio", "Rodar Attentus"],
        label="Escolha a opçao",
    ...
    if opcao == "Ajustar Polinomio" and \
        botao and arquivo_duracoes:
        fig = ajusta_polinomio(df_duracoes, grau_polinomio)
        st.pyplot(fig)
    elif opcao == "Rodar Attentus" and \
        botao and arquivo_ligantes and arquivo_duracoes:
